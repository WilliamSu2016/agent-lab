...
def _run_with_timeout(func, timeout_seconds: float):
    ...
    registers every worker thread it ever creates with a global ``atexit``
    hook that joins them all before the interpreter exits, even ones
    "shut down" without waiting -- so a hung real network call still blocks
    process exit long after its own timeout fired (discovered and fixed in
    experiment 4; see ``docs/04-PARALLEL-MULTI-AGENT.md``). A daemon thread
    has no such hook: the OS simply reclaims it whenever the process exits.
    """
...
def make_worker_node(text_llm_call: TextLLMCall):
    ...
    def research_worker(state: WorkerInput) -> MultiAgentResearchState:
        ...
        result: WorkerResult
        try:
            findings = _run_with_timeout(call_llm, timeout_seconds)
            result = {
