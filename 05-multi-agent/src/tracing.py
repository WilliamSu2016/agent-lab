...
*fully enabled* (agent runs, LLM generations, handoffs, tool calls/results, and
the final output are all still recorded -- Requirement 10, "保留完整 trace") but
replaces the default processor with one that writes trace/span data to a local
JSON-lines file. No third-party observability framework, no OpenTelemetry, and
no database are involved -- only the SDK's own
``TracingProcessor``/``TracingExporter`` extension points.
"""
...
def enable_local_tracing(trace_file: Path = DEFAULT_TRACE_FILE) -> Path:
    ...
    Every agent-run span (Triage, ResearchAgent, CodingAgent) and every
    handoff span shows up in the same trace, since they share one workflow run.
    """
