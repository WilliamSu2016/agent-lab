...
registered over MCP. Per the "do not add new business functionality"
constraint on this experiment, this file does not introduce one.
...
integration is added, it should be wired through this same
``src.reliability`` pipeline rather than a bespoke handler, and these
tests extended to call the real client instead of this generic stand-in.
"""
...
class TestMCPUnavailableFeedsResearcherStructuredFallback:
    ...
    Production Research run -- the researcher node converts it into a
    structured ``WorkerResult(status="failed")`` instead."""

    def test_researcher_reports_failed_status_instead_of_raising(self, monkeypatch, make_settings):
