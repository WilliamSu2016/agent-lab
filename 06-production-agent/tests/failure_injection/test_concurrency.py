...
never simulated sequentially) against the production primitives this
project already relies on for correctness under concurrent load:
    ...
    * ``src.api.rate_limit.RateLimiter`` -- concurrent callers sharing one
      ...
      the window, even when every check races every other one.
    * the full HTTP API (``src.api.app``) -- concurrent ``POST /runs``
      calls from different identities must never leak state across
      requests/threads (tenant isolation holds even under real concurrency,
...
class TestConcurrentIdempotentCalls:
    ...
    def test_only_one_of_many_concurrent_callers_actually_runs_the_side_effect(self):
        ...
        # Exactly one thread actually executed the real side effect...
        assert call_count["n"] == 1
        # ...every thread either got that one result or an explicit
        # in-progress signal -- never anything else, never a crash.
...
class TestConcurrentRateLimiting:
    ...
    def test_at_most_max_requests_succeed_under_concurrent_load(self):
        ...
        # Never more than the configured cap gets through, regardless of
        # how the 30 concurrent checks were interleaved.
    ...
    def test_different_keys_are_independent_under_concurrent_load(self):
        ...
        # 5 calls per key submitted, cap is 5 -- both keys fully admitted,
        # neither key's usage bled into the other's budget.
        assert results == {"user-a": 5, "user-b": 5}


class _ApiHarness:
    """Minimal API harness (mirrors ``tests/test_api.py``'s ``_Harness``)
    for exercising real concurrent HTTP requests against a durable graph
    without needing the full crash-injection machinery."""
    ...
    def __init__(self, tmp_path: Path):
        ...
        def graph_factory():
            return build_durable_graph(self.idempotency_store, checkpointer=self.saver)
...
class TestConcurrentHTTPRequestsPreserveTenantIsolation:
    ...
    different threads must never see each other's runs -- tenant isolation
    (Security experiment's Layer 2 requirement) must hold under genuine
    concurrency, not only sequential requests."""
        ...
        with TestClient(api_harness.app) as client:
            ...
            def create_for_tenant(tenant_id: str, index: int):
                ...
                response = client.post(
                    "/runs",
                    json={"question": f"question from {tenant_id} #{index}"},
            ...
            # Each tenant can read back only its own runs; every other
            # tenant's run_id is a 404 (indistinguishable from "unknown"),
