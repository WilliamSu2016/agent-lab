...
def _run_with_timeout(func, timeout_seconds: float):
    ...
    Delegates to the single canonical daemon-thread implementation in
    ``src/reliability/timeout.py`` (see that module's docstring for why a
    plain daemon thread is used instead of
    ``concurrent.futures.ThreadPoolExecutor``: a hung/slow call must never
    block process exit past its own timeout). Kept as a thin wrapper here
    (rather than calling ``run_with_timeout`` at each call site) so this
    module's own call sites/tests are unaffected by the consolidation --
    ``reliability.timeout.ToolTimeoutError`` is a ``TimeoutError`` subclass,
    so ``except TimeoutError`` handling here keeps working unchanged.
    """
    return run_with_timeout(func, timeout_seconds, tool_name="parallel_multi_agent_worker")
...
def main() -> None:
    ...
    try:
        outcome = run_parallel_research(
            ...
            max_total_seconds=settings.limits.run_timeout_seconds,
        )
