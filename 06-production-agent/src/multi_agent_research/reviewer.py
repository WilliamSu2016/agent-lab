...
Ownership: owns ONLY ``review``. It reads ``synthesis`` (and, for context,
``question``) but can never write ``synthesis`` itself -- if it finds a
problem, it must say so in ``review.feedback``/``review.missing_aspects``
and let the loop route control back to the Planner, which owns re-research
(same "read others, write only your own field" discipline as experiment 5).

Checks required by the task: completeness, factual consistency, evidence
quality, logical consistency, missing important aspects -- all five are
explicit fields on ``ReviewVerdict`` so a caller can inspect exactly which
dimension failed, not just a single opaque pass/fail bit.
"""
...
from src.multi_agent_research.state import MultiAgentResearchState, ReviewVerdict
...
def _parse_verdict(raw: str) -> ReviewVerdict:
    """Parse the Reviewer's JSON verdict, with a tolerant fallback.

    If the model does not return valid JSON, fail closed: reject with the
    raw text as feedback rather than silently approving a review we could
    not even parse.
    """
    text = raw.strip()
    if text.startswith("```"):
...
def make_reviewer_node(text_llm_call: TextLLMCall):
    """Node: Reviewer. Owns ONLY ``review``."""

    def reviewer(state: MultiAgentResearchState) -> MultiAgentResearchState:
...
def route_after_review(state: MultiAgentResearchState) -> str:
    """Conditional edge (not a node): PASS -> finalizer, FAIL -> planner
    (unless the iteration budget is exhausted, in which case we must stop
    regardless of the verdict -- Reliability requirement: max review
    iterations, so this loop can never run forever)."""
